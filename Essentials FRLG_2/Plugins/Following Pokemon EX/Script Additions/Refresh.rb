...
class Scene_Map
  ...
  def update(*args)
    ...
    # Don't toggle follower if in trade (prevents X button conflict)
    in_trade = defined?(pbModernTradeManager) && pbModernTradeManager &&
               pbModernTradeManager.current_trade_state

    if !in_trade &&
       defined?(FollowingPkmn::TOGGLE_FOLLOWER_KEY) && FollowingPkmn::TOGGLE_FOLLOWER_KEY &&
