class MultiplayerTrade_Scene
  ...
  def pbRunPictures(pictures, sprites)
    loop do      pictures.each { |pic| pic.update }
  ...
  def pbScene2
    ...
    picturePoke.setVisible(delay + cry_length + 4, true)
  ...
  def pbTrade
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
      ...
      pbMessageDisplay(@sprites["msgwindow"],
                       _INTL("{1}'s data was added to the Pokedex.", speciesname2)) { pbUpdate }
      $player.pokedex.register_last_seen(@pokemon2)
      pbFadeOutIn do        scene  =  PokemonPokedexInfo_Scene.new
...
def pbStartMultiplayerTrade(myPokemon, theirPokemon, myUsername, theirUsername)

  $stats.trade_count += 1

  theirPokemon.obtain_method = 2
  theirPokemon.record_first_moves

  pbFadeOutInWithMusic do    scene  =  MultiplayerTrade_Scene.new
    scene.pbStartScreen(myPokemon, theirPokemon, myUsername, theirUsername)
    scene.pbTrade
    scene.pbEndScreen
  end
end
