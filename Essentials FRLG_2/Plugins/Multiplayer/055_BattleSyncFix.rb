    ...
    Battle.class_eval do
      ...
      def pbCommandPhaseLoop(isPlayer)
        ...
        if isPlayer
          ...
          # Reseed RNG for deterministic battle behavior
          turn_seed = @multiplayer_battle_id.to_i + (@multiplayer_turn || 0) * 1000
          srand(turn_seed)
          puts "[MP SYNC] RNG seed: #{turn_seed}"
          ...
          if $multiplayer_opponent_choice_data && $multiplayer_opponent_choice_data.is_a?(Array)
            ...
            $multiplayer_opponent_choice_data.each do |choice_data|
              # Get choice details
              ...
              # Find opponent battler (swap target index for opponent's perspective)
              battler_idx = nil
