      ...
      begin
        ...
        if $multiplayer_created_at =~ /(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})/
          ...
          starttime = _INTL("{1} {2}, {3}",
                            pbGetAbbrevMonthName(created_time.mon),
                            created_time.day,
                            created_time.year)
        else
        ...
        # Fallback to local time if parse fails
        puts "[TrainerCard] Failed to parse server timestamp: #{e.message}"
